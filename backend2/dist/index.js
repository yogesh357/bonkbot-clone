...
bot.start(async (ctx) => {
    ...
    if (existingUser) {
        ...
        const { empty, message } = await getBalanceMessge(existingUser.publicKey.toString());
        ctx.reply(`Welcome back and here is your public key : ${publicKey} , now you can trade Solana with it.
    ...
    if (pending?.isPending) {
        ...
        // If mint is already set, this is the second message (amount)
            ...
            try {
                ...
                const quote = await getQuote("So11111111111111111111111111111111111111112", // SOL mint (fixed)
                pending.mint, // token mint user selected
                amountInSol * LAMPORTS_PER_SOL);
                if (!quote) {
                    ctx.reply("Failed to get quote. Please check the mint address and try again.");
                    delete PENDING_USER_BUYS[chatId];
                    return;
                }
                ...
                delete PENDING_USER_BUYS[chatId];
                ctx.reply(`✅ Swap Successful!\n\nYou can track it here: https://solscan.io/tx/${swapTxn}`, DEFAULT_KEYBOARD);
            }
            catch (error) {
